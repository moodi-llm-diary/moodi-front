...
프런트엔드는 browser-facing `VITE_API_BASE_URL`을 사용한다. 로컬과 Vercel에서는 이를 `/`로 둔다. 로컬은 `VITE_BACKEND_ORIGIN=http://localhost:8080`을 Vite same-origin proxy target으로 사용하고, Vercel은 `vercel.json`의 external rewrite로 `https://api.moodi.kro.kr`를 `/api/*` proxy target으로 사용한다. 브라우저에서 backend origin을 직접 호출하지 않아 GIS `g_csrf_token`, login-binding, host-only session cookie가 모두 같은 browser origin에 귀속되도록 한다. Google popup callback은 현재 프런트 origin의 `/api/v1/auth/google-credentials`를 same-origin service가 호출하며, Google OAuth client에는 배포 origin을 Authorized JavaScript origin으로 등록한다. 개발 서버와 Playwright의 browser origin은 backend·Google login 허용 목록과 일치하는 `http://localhost:5173`이며, `127.0.0.1` 또는 다른 포트로 접근하지 않는다. 모든 제품 API prefix는 `/api/v1`이며 field는 `camelCase` JSON이다.
...
| POST `/auth/google-credentials` | GIS credential form `credential`, `g_csrf_token`, `state` (popup callback 또는 redirect protocol) | `303` allow-listed returnTo + session cookie | 401 Google credential/attempt, 403 CSRF, 409 consumed attempt, 429 |
